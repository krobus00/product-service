...
clean-up-mock:
	...
	rm -rf ./pb/product/mock


pb/product/mock/mock_product_service_client.go:
	mockgen -destination=pb/product/mock/mock_product_service_client.go -package=mock ${PROJECT_REPO}/pb/product ProductServiceClient

# make generate
generate: clean-up-mock pb/product/mock/mock_product_service_client.go
	go generate ./...
...
proto:
	@protoc --go_out=. --go_opt=paths=source_relative --go-grpc_out=. \
  		--go-grpc_opt=paths=source_relative pb/product/*.proto
	@ls pb/product/*.pb.go | xargs -n1 -IX bash -c 'sed s/,omitempty// X > X.tmp && mv X{.tmp,}'
...
ifeq (dev server, $(filter dev server,$(MAKECMDGOALS)))
	...
	air --build.cmd 'go build -ldflags "-s -w -X main.version=$(VERSION) -X main.name=$(SERVICE_NAME)" -o bin/product-service main.go' --build.bin "./bin/product-service $(launch_args)"
...
build:
	# build binary file
	go build -ldflags "-s -w -X main.version=$(VERSION) -X main.name=$(SERVICE_NAME)" -o ./bin/product-service ./main.go
...
deploy:
	@helm upgrade --install product-service ./deployments/helm/server-product-service --set-file appConfig="${CONFIG}" --set app.container.version="${VERSION}" -n ${NAMESPACE}
